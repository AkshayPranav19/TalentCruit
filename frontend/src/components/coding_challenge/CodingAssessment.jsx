...
export default function CodingAssessment() {
  ...
  const handleResetCode = (questionNum) => {
    if (window.confirm("Are you sure you want to reset your code? This action cannot be undone.")) {
      const defaultCode = languageTemplates[questionNum] || "";
      setSolutions(prev => ({
        ...prev,
  ...
  const handleRunCode = async (questionNum) => {
    ...
    const code = solutions[questionNum] || languageTemplates[questionNum] || "";
    ...
    try {
      const res = await fetch("http://localhost:5002/run-tests", {
        ...
        body: JSON.stringify({ questionId: questionNum, code })
      });
  ...
  const handleSubmitAssessment = async () => {
    ...
    for (let q = 1; q <= totalQuestions; q++) {
      const code = solutions[q] || languageTemplates[q] || "";
        ...
        <div className="question-container">
          ...
          <div className="code-section">
            ...
            <Editor
              ...
              height="350px"
              language={language}
              value={solutions[currentQuestion] || languageTemplates[currentQuestion] || ""}
              theme="vs-dark"
              options={{
                fontSize: 14,
